...
namespace picovector {
  ...
  static int dither_level(pv_px p, int x, int y);
  ...
  void dither_brush_t::blend_spans(image_t *target, int i0, int i1, int step) {
    ...
    for(int i = i0; i < i1; i += step) {
      ...
      for(int w = spans[i].w; w; w--) {
        ...
        pv_r(p) = pv_g(p) = pv_b(p) = (uint8_t)v; // leave alpha
        p += PV_PX_STEP; x++;
      }
    }
  }
  ...
  void dither_brush_t::blend_masked_spans(image_t *target, int i0, int i1, int step) {
    ...
    for(int i = i0; i < i1; i += step) {
      ...
      for(int w = spans[i].w; w; w--) {
        ...
        int v = dither_level(p, x, y);
        pv_r(p) = (uint8_t)(pv_r(p) + (((v - pv_r(p)) * m) >> 8));
        pv_g(p) = (uint8_t)(pv_g(p) + (((v - pv_g(p)) * m) >> 8));
        pv_b(p) = (uint8_t)(pv_b(p) + (((v - pv_b(p)) * m) >> 8));
        p += PV_PX_STEP; x++;
      }
    }
  }
