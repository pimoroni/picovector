// Host tests for the pico3d mesh draw stage (transform + shading).
...
void test_pico3d_draw() {
  ...
  // 2) GOURAUD lit head-on is bright; light facing away leaves only ambient.
  {
    ...
    CHECK_MSG(lit_b > 700, "gouraud: head-on is bright");
    CHECK_MSG(dark_b < 100 && dark_b > 0, "gouraud: facing away leaves ambient only");
    CHECK_MSG(lit_b > dark_b, "gouraud: lit brighter than ambient");
  }
  ...
  // 3) FLAT shading produces a single uniform colour across the face.
  {
    ...
    CHECK_MSG(a != 0 && a == b, "flat: face colour is uniform");
  }
  ...
  // 4) Texture modulates the lit colour.
  {
    ...
    CHECK_MSG((c & 0xff) > 200 && ((c >> 8) & 0xff) == 0 && ((c >> 16) & 0xff) == 0, "texture: red texel applied");
  }
  ...
  //    nor depth; an opaque texel with the same cutoff draws normally.
  {
    ...
    pico3d_draw_mesh(&t, &m, &model, &vp, &mat, PICO3D_UNLIT, nullptr, VC);
    CHECK_MSG(color[center] == 0, "alpha cutout: transparent texel discarded (no colour)");
    CHECK_MSG(depth[center] == 0xFFFF, "alpha cutout: transparent texel writes no depth");
    ...
    CHECK_MSG(((color[center] >> 8) & 0xff) > 200, "alpha cutout: opaque texel drawn");
  }
  ...
  // 6) per-vertex colours override material colour (the flat-facet path).
  {
    ...
    uint32_t c = color[center];
    CHECK_MSG((c & 0xff) < 60 && ((c>>8)&0xff) > 120 && ((c>>16)&0xff) < 80,
